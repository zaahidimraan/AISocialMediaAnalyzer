...
def conduct_research(state: AgentState):
    ...
    existing_urls = state.get('visited_urls', [])
    
    print(f"--- RESEARCHING: {query} ---")
    
    # Call the tool with the existing history
    search_result = search_web(query, existing_urls)
    
    # Return updates to the state
    return {
        "latest_content": search_result["content"],
        # Because we used operator.add in state.py, 
        # these new URLs will be appended to the existing list automatically.
